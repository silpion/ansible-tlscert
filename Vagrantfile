...
Vagrant.configure(VAGRANT_API_VERSION) do |config|
  ...
  # vm configuration
  config.vm.box = ENV['ANSIBLE_TLSCERT_VAGRANT_BOXNAME'] || c['vm']['box']
  config.vm.box_check_update = c['vm']['check_update']

  config.vm.define :ansibletlscerttest do |d|

    d.vm.hostname = 'ansibletlscerttest'
    ...
    d.vm.provision :ansible do |ansible|
      ...
      ansible.groups = {
        'vagrant' => ['ansibletlscerttest']
      }
      ansible.limit = 'vagrant'

      #   dynamic ansible-playbook configuration based on environment variables
      ansible.tags = ENV['ANSIBLE_TLSCERT_VAGRANT_ANSIBLE_TAGS']
      ansible.skip_tags = ENV['ANSIBLE_TLSCERT_VAGRANT_ANSIBLE_SKIP_TAGS']
      #   dynamic ansible-playbook configuration based on environment variables or user configuration
      ansible.verbose = ENV['ANSIBLE_TLSCERT_VAGRANT_ANSIBLE_VERBOSE'] || c['provisioner']['ansible']['verbose']

      # ansible-playbook raw arguments
      ansible.raw_arguments = []

      if ENV['ANSIBLE_TLSCERT_VAGRANT_ANSIBLE_CHECKMODE'] == '1'
        ansible.raw_arguments << '--check'
      end

      if ENV['ANSIBLE_TLSCERT_VAGRANT_ANSIBLE_DIFFMODE'] == '1' or c['provisioner']['ansible']['diff']
    ...
    d.vm.provider :virtualbox do |v|
      ...
      v.customize [ 'modifyvm', :id, '--name', 'ansibletlscerttest', '--memory', '1024', '--cpus', '1' ]
    end
